...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            indxE = assignment(:, indx) == x;
            observedProbs = F(j).val(indxE)/sum(F(j).val(indxE));
            F(j) = SetValueOfAssignment(F(j), assignment(indxE), observedProbs);

            indxNotE = assignment(:, indx) != x;
            F(j) = SetValueOfAssignment(F(j), assignment(indxNotE), zeros(1, length(F(j).val) - length(observedProbs)));
